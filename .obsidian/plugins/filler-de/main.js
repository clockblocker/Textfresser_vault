...
`;var Ee="Translate the given German text to English";var xe=`You are a German language expert. Translate the given Russian text to grammatically correct German, maintaining the original meaning and using natural German expressions.
<example>
input:
...
`))}catch(s){new Se.Notice(`Error: ${s.message}`)}}var Ie=require("obsidian");async function j(e,n,t,i){let r=await b(n);if(!r)return;let s=await G(t);if(s)try{if(t.file){let a=await e.app.vault.read(t.file),u=e.findHighestNumber(a)+1,d=await e.apiService.makeBrackets(r);if(d){let m=i?await L(d,s,u):d;n.replaceSelection(`${m}`)}}}catch(a){new Ie.Notice(`Error: ${a.message}`)}}var Ae=require("obsidian");async function Z(e,n){let t=await b(n);if(t)try{let i=n.getCursor(),r=await e.apiService.translateText(t);r&&(n.replaceSelection(t+`

`+r+`
`),n.setCursor({line:i.line,ch:i.ch+t.length}))}catch(i){new Ae.Notice(`Error: ${i.message}`)}}var Ne=require("obsidian");async function X(e,n,t){let i=await b(n);if(!i)return;let r=await G(t);if(r)try{if(t.file){let s=await e.app.vault.read(t.file),o=e.findHighestNumber(await s)+1,u=await L(i,r,o);await navigator.clipboard.writeText(`${u} 

...
`)}}catch(s){new Ne.Notice(`Error: ${s.message}`)}}var Re=require("obsidian");async function Q(e,n){let t=await b(n);if(t)try{let i=await e.apiService.checkRuDeTranslation(t);i&&n.replaceSelection(t+`
`+i+`
`)}catch(i){new Re.Notice(`Error: ${i.message}`)}}var H=class extends p.Plugin{async onload(){try{await this.reloadPlugin(),this.addSettingTab(new T(this.app,this))}catch(t){console.error("Error during plugin initialization:",t),new p.Notice(`Plugin failed to load: ${t.message}`)}}async reloadPlugin(){await this.loadSettings(),this.apiService=new F(this.settings,this.app.vault),this.fileService=new P(this.app.vault),this.addCommand({id:"backlink-all-to-current-file",name:"Add backlinks to the current file in all referenced files",editorCallback:async(t,i)=>{var m,w;let r=(m=i.file)==null?void 0:m.name;if(!i.file||!r){new p.Notice("Current file is missing a title");return}let{metadataCache:s,vault:a}=this.app,o=s.getFileCache(i.file),l=(w=o==null?void 0:o.links)!=null?w:[],d=[];for(let c of l){let u=c.link,h=s.getFirstLinkpathDest(u,i.file.path);h instanceof p.TFile?d.push({name:u,path:h.path}):d.push({name:u,path:null})}for(let c of d)try{let u,h=`[[${r.split(".")[0]}]]`;if(c.path)u=c.path;else{let Te=c.name[0].toUpperCase(),$=(0,p.normalizePath)(`Worter/${Te}`);a.getAbstractFileByPath($)||await a.createFolder($),u=(0,p.normalizePath)(`${$}/${c.name}.md`)}await this.fileService.doesFileContainContent(u,h)||await this.fileService.appendToFile(u,`, ${h}`)}catch(u){new p.Notice(`Error processing link ${c.name}: ${u.message}`)}}}),this.addCommand({id:"fill-template",name:"Generate an dictionary entrie for the word in the title of the file",editorCallback:async(t,i)=>{await q(this,t,i)}}),this.addCommand({id:"get-infinitive-and-emoji",name:"Get infinitive form and emoji for current word",editorCallback:async(t,i)=>{await J(this,t,i)}}),this.addCommand({id:"duplicate-selection",name:"Add links to normal/inf froms to selected text",editorCallback:async(t,i)=>{await j(this,t,i,!0)}}),this.addCommand({id:"normalize-and-do-not-link",name:"z: Add liks to normal/inf forms [W/O a source link]",editorCallback:async(t,i)=>{await j(this,t,i,!1)}}),this.addCommand({id:"translate-selection",name:"Translate selected text and show below",editorCallback:async t=>{await Z(this,t)}}),this.addCommand({id:"format-selection-with-number",name:"Format selection with next number and source link",editorCallback:async(t,i)=>{await X(this,t,i)}}),this.addCommand({id:"translate-ru-to-de",name:"Translate Russian text to German",editorCallback:async t=>{let i=t.getSelection();if(!i){new p.Notice("No text selected");return}try{let r=await this.apiService.translateRuToDe(i);r&&t.replaceSelection(i+`
`+r+`

...
`+i+`
`)}catch(i){new Ne.Notice(`Error: ${i.message}`)}}var H=class extends f.Plugin{async onload(){try{await this.reloadPlugin(),this.addSettingTab(new N(this.app,this))}catch(t){console.error("Error during plugin initialization:",t),new f.Notice(`Plugin failed to load: ${t.message}`)}}async reloadPlugin(){await this.loadSettings(),this.apiService=new M(this.settings,this.app.vault),this.fileService=new F(this.app.vault),this.addCommand({id:"backlink-all-to-current-file",name:"Add backlinks to the current file in all referenced files",editorCallback:async(t,i)=>{var m,w;let r=(m=i.file)==null?void 0:m.name;if(!i.file||!r){new f.Notice("Current file is missing a title");return}let{metadataCache:s,vault:a}=this.app,o=s.getFileCache(i.file),l=(w=o==null?void 0:o.links)!=null?w:[],c=[];for(let d of l){let u=d.link,h=s.getFirstLinkpathDest(u,i.file.path);h instanceof f.TFile?c.push({name:u,path:h.path}):c.push({name:u,path:null})}for(let d of c)try{let u,h=`[[${r.split(".")[0]}]]`;if(d.path)u=d.path;else{let Te=d.name[0].toUpperCase(),j=(0,f.normalizePath)(`Worter/${Te}`);a.getAbstractFileByPath(j)||await a.createFolder(j),u=(0,f.normalizePath)(`${j}/${d.name}.md`)}await this.fileService.doesFileContainContent(u,h)||await this.fileService.appendToFile(u,`, ${h}`)}catch(u){new f.Notice(`Error processing link ${d.name}: ${u.message}`)}}}),this.addCommand({id:"fill-template",name:"Fill the template for the word in the title of the file",editorCallback:async(t,i)=>{await W(this,t,i)}}),this.addCommand({id:"get-infinitive-and-emoji",name:"Get infinitive form and emoji for current word",editorCallback:async(t,i)=>{await q(this,t,i)}}),this.addCommand({id:"duplicate-selection",name:"Add links to normal/inf froms to selected text",editorCallback:async(t,i)=>{await K(this,t,i,!0)}}),this.addCommand({id:"normalize-and-do-not-link",name:"z: Add liks to normal/inf forms [W/O a source link]",editorCallback:async(t,i)=>{await K(this,t,i,!1)}}),this.addCommand({id:"translate-selection",name:"Translate selected text and show below",editorCallback:async t=>{await J(this,t)}}),this.addCommand({id:"format-selection-with-number",name:"Format selection with next number and source link",editorCallback:async(t,i)=>{await Z(this,t,i)}}),this.addCommand({id:"translate-ru-to-de",name:"Translate Russian text to German",editorCallback:async t=>{let i=t.getSelection();if(!i){new f.Notice("No text selected");return}try{let r=await this.apiService.translateRuToDe(i);r&&t.replaceSelection(i+`
`+r+`

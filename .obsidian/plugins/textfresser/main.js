...
`;var I=require("obsidian"),w="\u2014";function Q(e,n,t){return e=e.replace(/[\s\n]+$/,""),`${`[[${n}#^${t}|^]]`} ${e} ^${t}
...
`),await navigator.clipboard.writeText(`${d}`),n.replaceSelection(`${d}`)}catch(s){console.error("Error formatting selection with number:",s)}}var C=require("obsidian");async function le(e,n,t,i,r){var a;try{let s=i.getFileCache(e),u=(a=s==null?void 0:s.links)!=null?a:[];console.log("links",u);let l=[];for(let d of u){let h=d.link,g=i.getFirstLinkpathDest(h,e.path);g instanceof C.TFile?l.push({name:h,path:g.path}):l.push({name:h,path:null})}for(let d of l)try{let h,g=`[[${n}]]`;if(d.path)h=d.path;else{let o=d.name[0].toUpperCase(),p=(0,C.normalizePath)(`Worter/${o}`);t.getFolderByPath(p)||await t.createFolder(p),h=(0,C.normalizePath)(`${p}/${d.name}.md`)}await L(t,h,g)||await H(t,h,`, ${g}`)}catch(h){new C.Notice(`Error processing link ${d.name}: ${h.message}`)}r.refresh()}catch(s){new C.Notice(`Error processing backlinks: ${s.message}`)}}async function ce(e,n,t){try{let i=await e.apiService.consultC1Richter(t);i&&n.replaceSelection(t+`
`+i+`

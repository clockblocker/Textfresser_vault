...
---
`)+`
`:e}catch(n){return console.error("Failed to read clipboard:",n),e}}async function re(e,n,t,i){let r=t.basename;try{let[s,a,u,d]=await Promise.all([e.apiService.generateContent(k.generate_dictionary_entry,r),e.apiService.generateContent(k.generate_forms,r),e.apiService.generateContent(k.morphems,r),e.apiService.generateContent(k.generate_valence_block,r)]),c=Xn(a),m=`${s.replace("<agent_output>","").replace("</agent_output>","")}`,g=await qn(Jn(m)),h=u.replace(`
`,"")===b?"":`${u}
`,o=d.replace(`
`,"")===b?"":`${d}`,p=a.replace(`
`,"")===b?"":`${a}`,y=c.replace(`

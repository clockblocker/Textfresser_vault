...
---
`)+`
`:e}catch(n){return console.error("Failed to read clipboard:",n),e}}async function ee(e,n,t,i){let r=t.basename;try{let[a,s,u,l]=await Promise.all([e.apiService.generateContent(b.generate_dictionary_entry,r),e.apiService.generateContent(b.generate_forms,r),e.apiService.generateContent(b.morphems,r),e.apiService.generateContent(b.generate_valence_block,r)]),d=Yn(s),p=`${a.replace("<agent_output>","").replace("</agent_output>","")}`,h=await Un($n(p)),c=u.replace(`
`,"")===w?"":`${u}
`,o=l.replace(`
`,"")===w?"":`${l}`,g=s.replace(`
`,"")===w?"":`${s}`,x=d.replace(`
`,"")===w?"":`${d}`,y=[h,c,o,g,x].filter(Boolean).join(`
---
`),k=He(h);(k==null?void 0:k.toLocaleLowerCase())===r.toLocaleLowerCase()?await e.fileService.appendToFile(t.path,y):(await e.fileService.appendToFile(t.path,`[[${k}]]`),await navigator.clipboard.writeText(y))}catch(a){new We.Notice(`Error: ${a.message}`)}}function Vn(e){let n=e.match(/Adjektive:\s*\[\[(.*?)\]\],\s*\[\[(.*?)\]\],\s*\[\[(.*?)\]\]/);if(!n)return null;let[t,i,r,a]=n;return[i,r,a]}function Yn(e){let n=Vn(e);if(!n)return w;let t=["er","es","e","en","em"],i=[];for(let r of n)for(let a of t)i.push(`[[${r+a}]]`);return i.join(", ")}var Le=require("obsidian");async function ne(e,n,t){let i=t.basename;try{let r=await e.apiService.determineInfinitiveAndEmoji(i);r&&(r=r.replace(/^\n+/,""),r=r.trim(),await e.fileService.appendToFile(t.path,r+`
...
`+r+`
`),n.setCursor({line:i.line,ch:i.ch+t.length}))}catch(i){new Ue.Notice(`Error: ${i.message}`)}}var en=un(Qe());async function ue(e,n,t,i){let r=t.basename,a=(0,en.sentences)(i,{preserve_whitespace:!1,newline_boundaries:!1,html_boundaries:!1,sanitize:!0});try{let s=n.getValue(),l=e.findHighestNumber(s)+1,d="";if(a.length<=2)d=Q(i,r,l);else{let p=[];for(let h=0;h<a.length;h++){let o=a[h].match(/(?:[^\n]+|\n+)/g);o&&p.push(o.map(g=>g.trim()?Q(g.trim(),r,l+h):g).join(""))}d=p.join(`
`)}await navigator.clipboard.writeText(`${d}`),n.replaceSelection(`${d}`)}catch(s){console.error("Error formatting selection with number:",s)}}var S=require("obsidian");async function le(e,n,t,i,r){var a;try{let s=i.getFileCache(e),u=(a=s==null?void 0:s.links)!=null?a:[];console.log("links",u);let l=[];for(let d of u){let p=d.link,h=i.getFirstLinkpathDest(p,e.path);h instanceof S.TFile?l.push({name:p,path:h.path}):l.push({name:p,path:null})}for(let d of l)try{let p,h=`[[${n}]]`;if(d.path)p=d.path;else{let o=d.name[0].toUpperCase(),g=(0,S.normalizePath)(`Worter/${o}`);t.getFolderByPath(g)||await t.createFolder(g),p=(0,S.normalizePath)(`${g}/${d.name}.md`)}await L(t,p,h)||await H(t,p,`, ${h}`)}catch(p){new S.Notice(`Error processing link ${d.name}: ${p.message}`)}r.refresh()}catch(s){new S.Notice(`Error processing backlinks: ${s.message}`)}}async function ce(e,n,t){try{let i=await e.apiService.consultC1Richter(t);i&&n.replaceSelection(t+`
`+i+`
